...
<div class="col-sm-8 col-sm-offset-2 costum">
    ...
    <table style="width:100%" class="table-hoverr">
        <div class="button-group">
            ...
            <a href="{{ route('pages.create') }}" class="btn btn-primary">צור דף חדש מתוך דוגמאות <bold>+</bold></a>
        </div>
            ...
            @if(count($pages))
                @foreach($pages as $page)
                    <tr>
                        ...
                        <td> {{ $page->title }} </td>
                        <td> {{ str_limit($page->body,35,'.....') }} </td>
                        <td> {{ $page->user->first_name }} </td>
                        <td> {{ $page->user->permission? $page->user->permission : "User" }} </td>
